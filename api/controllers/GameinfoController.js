    ...
    async.waterfall([
          ...
          async.whilst(
            ...
            function (callback) {
              async.waterfall([
                ...
                function getQuest3PlayersPassed(obj, callback) {
                  if (obj.quest3 === 0) {
                    callback(null, obj);
                  } else {
                    gcmainconn.query('SELECT COUNT(*) as `count` FROM `quests_data` WHERE (`data` LIKE \'%"quest_3":"1"%\' OR `data` LIKE \'%"quest_3":1%\') AND `user` IN (?)', [obj.playersIds], function (err, result) {
                      if (err) return callback(err);

                      obj.quest3 = result[0].count;

                      callback(null, obj);
                    });
                  }
                },
                function getQuest4PlayersPassed(obj, callback) {
                  if (obj.quest4 === 0) {
                    callback(null, obj);
                  } else {
                    gcmainconn.query('SELECT COUNT(*) as `count` FROM `quests_data` WHERE (`data` LIKE \'%"quest_4":"1"%\' OR `data` LIKE \'%"quest_4":1%\') AND `user` IN (?)', [obj.playersIds], function (err, result) {
                      if (err) return callback(err);

                      obj.quest4 = result[0].count;
      ...
      function serializeData(obj, callback) {
        ...
        async.each(obj.data, function (element, callback) {
          ...
          quest3.push([moment(element.date).add(4, 'hours').unix() * 1000, element.quest3]);
          quest4.push([moment(element.date).add(4, 'hours').unix() * 1000, element.quest4]);
